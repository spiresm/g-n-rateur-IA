...
import type { PosterParams, GenerationParams, GeneratedImage } from '../App';
...
// -----------------------------
// APP CONTENT (Header + Carousel + Container logique)
// -----------------------------
export function AppContent() {
  ...
  // Pour permettre à PosterGenerator de “donner” une fonction au parent si tu en as besoin
  const generateFnRef = useRef<null | (() => void)>(null);

  // Petit helper: fetch JSON
  ...
  // Poll /progress jusqu’à completed=true
  ...
  // Récupère le résultat base64 et le convertit en dataURL
  ...
  // Ton “onGenerate” : déclenche le bridge → poll → result → push dans carousel
  const onGenerate = useCallback(
    async (posterParams: PosterParams, genParams: GenerationParams) => {
      ...
      try {
        // Le bridge attend du form-data (x-www-form-urlencoded marche aussi si ton bridge le supporte).
        // On envoie au minimum prompt + params.
        const body = new URLSearchParams();

        // Champs “safe” (placeholders communs)
        ...
        // Optionnel: si ton workflow utilise des placeholders liés à l’affiche
        ...
        // 1) /generate
        const gen = await fetchJson(`${BACKEND_URL}/generate?workflow_name=${encodeURIComponent(DEFAULT_WORKFLOW)}`, {
          method: 'POST',
        ...
        // 2) poll /progress
        const generationTime = await waitForCompletion(promptId);

        // 3) /result
        const imageUrl = await fetchResultImage(promptId);

        // push dans le carousel
  ...
  const headerSubtitle = useMemo(() => {
    ...
    return `Backend: ${host} · Workflow: ${DEFAULT_WORKFLOW}`;
  }, []);
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-800/40 border border-gray-700 rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-xs text-gray-400">{generatedImages.length} image(s)</div>
          </div>
                ...
                >
                  ...
                  <div className="p-3">
                    <div className="text-xs text-gray-300 line-clamp-2">
                      {img.params?.final_prompt}
                    </div>
                    <div className="mt-2 text-[11px] text-gray-500">
                      {img.generationTime ? `${Math.round(img.generationTime / 1000)}s` : ''} · {img.timestamp.toLocaleString()}
                    </div>
                  </div>
                </div>
              ))}
...
// -----------------------------
// POSTER GENERATOR (ton composant existant)
// -----------------------------
...
export function PosterGenerator({
  ...
  // Construire prompt + params et appeler onGenerate
  const handleGenerate = useCallback(() => {
    ...
    // Prompt final (à adapter si tu as déjà un builder ailleurs)
    const final_prompt = [
      `Movie poster`,
      title ? `Title: ${title}` : '',
