...
export const Header = memo(function Header() {
  ...
  // ✅ Correction Photo Google : Supabase utilise user_metadata.avatar_url
  const avatarUrl = user?.user_metadata?.avatar_url || user?.photoURL;
  const displayName = user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'Artiste';

  return (
    <>
      <header className="fixed top-0 left-0 right-0 h-24 sm:h-32 bg-gray-800 border-b border-gray-700 flex items-center justify-between px-4 sm:px-8 z-50">
        <div className="flex items-center gap-4">
          ...
          <div className="hidden xs:flex flex-col text-left">
            ...
            <p className="text-[10px] text-gray-500 font-bold tracking-widest uppercase mt-1">Studio de création</p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3 sm:gap-6 ml-auto">
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-gray-800">
              <div className="hidden lg:flex flex-col items-end">
                ...
                <span className="text-xs text-gray-200 font-medium truncate max-w-[120px]">{displayName}</span>
              </div>
